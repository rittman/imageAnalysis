...
mainDir = getcwd()
SPmedian = open('/scratch/rb729/PDStudy/SP_median.txt','w') #this is creating a new text file which we will use to write the output of the script to. 'w' stands for 'write' because we are going to be writing to this file as opposed to reading from it.   
...
for d in ['Control','PD']: #for loop for d. d is either control or PD as a result.
	#print d
	path3 = '/scratch/rb729/PDStudy/'+d #this says where to find d
	directory = listdir(path3) #listdir(path3) is a function to list path3, in this case the list only consists of the 'Control' folder and the 'PD' folder so our variable 'directory' pretty much defines these two folders. 
	...
	for WBIC in directory: #now we move one folder deeper to the WBIC number. 'WBIC' here represents all the wbic numbers in this for loop. 
		#print WBIC
		path1 = '/scratch/rb729/PDStudy/'+d+'/'+WBIC #this tells us where to find the WBIC folders
		subdirectory = listdir(path1) #again, here we are making a variable which is defined by the list of all the WBIC folders. 

		for condition in subdirectory: #now we're going into the condition folder where ultimately we'll be able to find the original spike percentage text which tells us the spike percentage for each frame of the scan. 
			try:
				#print condition
				#print getcwd()
				#print mainDir
				#print d
				#print WBIC
				chdir(path.join(mainDir, d, WBIC, condition))
				SP = open(WBIC+'_functional_reordered_pp_thld7_SP.txt','r') #this is opening the text file with the spike percentage information for each frame. 'r' means that we're reading from the file as oppossed to writing to it as we are doing with the SPmedian text file. 	
...
# get proportion surviving
	...
	##		z = 0.0       #this is an old way of calculating median, see line above for the function however you need to import from numpy at top
	##		for x in SPlist: 
	##			z += x
	##		mu = z / len(SPlist)
				#print mu
				
				SPmedian.write('\t'.join([WBIC,d+'\t',condition+'\t',str(mu)+'\n'])) #this writes each rowe of our new SPmedian text file. first column wbic number, then tab, then condition, then tab, then median, then new line which is indicated with '\n'. 
